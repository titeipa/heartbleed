...
def thread_check():
    global counter_nossl, counter_notvuln, counter_vuln
    while True:
        # get some work
        rank, domain = work_q.get()
        if domain is None:
            sys.stdout.write('Good!')
            work_q.task_done()
            work_q.put(domain) # forword
            # thread.exit()
            break
...
def main():
    ...
    threads = []
    for i in range(100):
        threads.append(Thread(target = thread_check))
        threads[-1].start()
    ...
    for line in f:
        ...
        domain = domain.strip()
        work_q.put([rank, domain])
        
        if int(rank) >= int(args[1]):
            break

    work_q.put(None) # signal end of tasks
